...
Private Sub cmdgenerar_Click()
  ...
  selecte = "select cod_egr,fecha,format(sucursal,'0000')&'-'&format(n_comp,'00000000') as numero," & _
            "nom_comp,nom_prov,format(cuit_prov,'00-00000000-0') as cuit_prov1," & _
            "iif(e.cod_cue is null,'indefinido',nom_cue) as cuenta," & _
            "gravado,no_gravado,iva21,iva105,iva27,exento,perc_iva,perc_ib,interno," & _
            "gravado+no_gravado+format(iva21+iva105+iva27,'0.00')+exento+perc_iva+perc_ib+interno as subtotal " & _
            "from (((egresos" & txtemp & " as e " & _
            "left join cuentas as cu on cu.cod_cue=e.cod_cue) " & _
            "inner join proveedores as pr on pr.cod_prov=e.cod_prov) " & _
            "inner join comprobantes as co on co.cod_comp=e.letra) " & _
            "where true"
  If sprov <> "" Then selecte = selecte & " and e.cod_prov in " & sprov
  If scue <> "" Then selecte = selecte & " and e.cod_cue in " & scue
  If cmbletra.ListIndex > -1 Then selecte = selecte & " and e.letra=" & cmbletra.ListIndex
